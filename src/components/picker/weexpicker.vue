      ...
      <div class="picker-list-wrapper" ref="wrapper">
        <div class="picker-list">
          ...
          <list class="picker-list-item">
            ...
            <cell><div class="picker-panel"></div></cell>
            <cell v-for="(item, index) in hourState.data"
                                  :key="index" :ref="'hour'+index">
          ...
          <list class="picker-list-item">
            <cell><div class="picker-panel"></div></cell>
            <cell><div class="picker-panel"></div></cell>
            <cell v-for="(item, index) in minuteState.data"
                                  :key="index" :ref="'minute'+index">
          ...
          <!--<list class="picker-list-item">
            <cell v-for="(item, index) in secondState.data"
                                  :key="index">
              <div class="picker-panel list-bg" :style="{marginTop: !!item.devide ? '30px':'0',backgroundColor: item.current ? '#008dfc' : '#fff'}" @click="getSelectedData(index, 'second')">
                <text class="picker-text" :style="{color: item.current ? '#fff' : '#008dfc'}">{{item.value}}</text>
              </div>
            </cell>
          </list>-->
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      pickerOk () {
        this.$emit('ok', this.current['hour']['value']+':'+this.current['minute']['value'])
      },
      wrapper () {
